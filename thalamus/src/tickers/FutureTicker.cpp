// VVV FILE: ./thalamus/src/tickers/FutureTicker.cpp VVV
...
// Fair value calculation (via Cost of Carry formula)
void FutureTicker::recalculate_fair_value() {
    // 1. Retrieve live Risk-Free Rate
    MacroData macro = TickerRegistry::get_macro_data();
    double r = macro.risk_free_rate; 

    // 2. Retrieve the underlying Spot Ticker
    ...
    // Pull data from the spot ticker's state (Uses the SCARCITY adjusted fair value, not the raw market price)
    ...
    // NEW: Safe guard against expired or expiring-today contracts. Cap at minimum 1 hour (0.0001 yrs)
    if (days <= 0) {
        current_state.time_to_expiry = 0.0001; 
    } else {
        current_state.time_to_expiry = days / 365.0;
    }

    // 4. Storage Cost Calculation (u)
    // If supply chains are healthy (1.0), storage is cheap (2%).
    // If supply chains collapse (0.1), hoarding begins and storage costs spike.
    double base_storage = 0.02; 
    double storage_cost = base_storage / std::max(0.1, flow_health); 

    // 5. Continuous Cost of Carry Model: F = S * e^((r + u) * t)
    // Note: Convenience yield 'y' is implicitly handled by the Scarcity Premium already built into 'S' above.
    double cost_of_carry = r + storage_cost;
    
    current_state.fair_value = S * std::exp(cost_of_carry * current_state.time_to_expiry);

    // 6. Implied Carry (Arbitrage detection)
...
// ^^^ END FILE: ./thalamus/src/tickers/FutureTicker.cpp ^^^